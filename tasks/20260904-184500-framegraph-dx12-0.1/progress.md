...
Before/after MSVC Release data uses 31 samples × 1,000 operations after warmup. Compile moved 52.5261→40.1816 µs (-23.50%); alias-on barrier planning moved 26.9801→25.1527 µs (-6.77%). The only production change reserves local/output vectors from already validated bounds, using actual retained counts for persistent result capacity. Plan identity stayed `87a12f2fdda63447`; memory and 192 transitions/64 UAV/64 activation barriers are exactly unchanged. Allocation stayed about 2.2 µs and was not modified. Barrier audit found no provably redundant operation, so none was removed.

Seven paired clean PACT-70 hardware runs at 1280x720×240 frames observed 13,893,632 alias-on versus 17,825,792 alias-off actual bytes (-3,932,160, -22.06%). Median graph-work GPU time was 0.106482 versus 0.104865 ms and CPU record was 0.092351 versus 0.089053 ms. Every pair matched pixels/counts and Debug 0/0/0. The small timing deltas do not justify weakening activation safety; no SLA is claimed.

Final reserve code passed MQB Release 100,000 valid + 100,000 invalid independent full-plan graphs and 100,000 bounded mutations. The initial reserve version also passed the 100k+100k sweep under Clang ASan+UBSan; the final refinement passed sanitized CTest 12/12. Current CMake/MSVC Debug/Release are 13/13; portable Clang, the local g++-named LLVM driver and Sanitizer profiles are 12/12. Raw paths, exact environment and limitations are recorded in `evidence/tier2-results.md`. Tier 2 checkpoint/push and clean-head rerun remain; Tier 3 reliability follows.
