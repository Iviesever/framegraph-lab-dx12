...
#include <array>
namespace fgl {
struct SceneState { float yaw{0.7f}; std::uint32_t logical_frame{}, debug_view{}, seed{24301}; bool paused{}, single_step{}; };
class SceneRenderer {
public:
    SceneRenderer(ID3D12Device* device, const std::filesystem::path& shader_directory);
    void depth(Dx12PassContext& pass, framegraph::ResourceId target, const SceneState& state);
    void scene(Dx12PassContext& pass, framegraph::ResourceId target, framegraph::ResourceId depth, const SceneState& state);
...
struct SceneProgram {
    ...
    framegraph::ResourceId backbuffer, readback;
};
