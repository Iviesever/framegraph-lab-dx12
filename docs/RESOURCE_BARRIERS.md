...
`Usage::IndirectArgument` is a read-only buffer use. Core maps it to `ResourceState::IndirectArgument`; D3D12 maps that state to `D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT`. The scene plan therefore contains UAV ordering after culling, UAV→SRV for visible IDs, UAV→Indirect for draw arguments, and Indirect→CopySource before count readback. `ExecuteIndirect` does not perform hidden state inference.
